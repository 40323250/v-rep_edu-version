...
//		bool success=(buttonID!=-1);
	...
	if (_rawCmdID==simx_cmd_call_script_function)
	{
		...
		if (inBufferSize>0)
			D.pushOutData_luaFunctionCall(CLuaFunctionDataItem(_pureData+off,inBufferSize));
		else
		...
		if (simCallScriptFunction(options,functionName.c_str(),&c,NULL)!=-1)
		{ // success!
			...
			if (D.readDataFromLua_luaFunctionCall(&c,outArgs,outArgs[0],functionName.c_str()))
			{
				...
				int outIntCnt=(int)outData->at(0).intData.size();
				...
				for (int i=0;i<outIntCnt;i++)
					appendIntToString(retData,outData->at(0).intData[i],true,otherSideIsBigEndian);
